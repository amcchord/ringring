...
If an operator adds Prometheus or another scraper, keep the loopback boundary and collect through a deliberately designed host-side mechanism; the production app and `ringringctl` reject a non-loopback `METRICS_ADDR`. Require authentication at every human-facing dashboard, avoid service discovery that publishes container ports, and choose the shortest useful retention. Do not add raw Asterisk events, contact details, SIP messages, HTTP paths, query strings, cookies, request bodies, provider payloads, audio, or transcripts as labels or logs.
