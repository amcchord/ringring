...
| T11 | Radio/weather input becomes SSRF or command/config injection | Radio stores a catalog identifier and both the web layer and renderer resolve it to a fixed source-controlled URL. Weather uses a bounded host-entered place or exactly five DTMF ZIP digits through an HTTPS API client. Phone setup authenticates the endpoint inside its party and can fill only an unknown, host-allowed location; input never becomes a URL, shell command, or Asterisk application name. | Open-Meteo learns the requested place. OpenAI receives fixed setup text and the resolved label in fixed forecast speech. External providers may be unavailable or compromised. Radio content is not moderated by RingRing. |
