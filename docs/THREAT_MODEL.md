...
| T04 | Attacker registers as a family phone | Each device has an independent CSPRNG-generated identity and password. New identities are 15 nonzero-leading decimal digits (about 49.7 bits of global space); new passwords are 24 such digits (about 79.6 bits), retaining offline-guessing resistance while remaining keypad-enterable. Authenticated username precedes IP/anonymous identification. Fail2Ban and separate UDP/TLS limits slow online guessing; rotation and revocation regenerate live configuration. | UDP SIP exposes a digest exchange to an on-path observer; old adapters may have weak TLS support. Prefer TLS. Existing credentials retain their original stronger format until deliberately rotated. |
...
| T09 | Invitation or Linphone setup link/QR is replayed, logged, or grants excess access | Tokens have 32 random bytes, are stored hashed, expire, and are atomically consumed once. Both QR images are rendered in memory without a third-party service. The host invitation reveal is consumed once on a no-store authenticated page; a host-scoped transaction can count and cancel only that party's active unused links without selecting hashes. Only a successful claim response gets a reduced first-call snapshot of dialable display labels/extensions and enabled lines; the invite form, host setup cards, and provisioning XML do not. Provisioning returns one device's technical fields only. Token-bearing routes are represented by templates in logs and pages use restrictive referrer/cache policy. | Anyone who obtains an unused bearer link or photograph of its QR before the recipient can consume it. The host must share it privately and cancel active links or rotate a claimed device after suspected disclosure. A successful claimant becomes a party member and can see the same party labels/extensions needed to place calls. |
...
| T15 | Vulnerable dependency or unreviewed boundary change ships | `make check` runs race tests, vet, shell syntax checks, admin tests, and the boundary contracts. `make security` adds pinned `govulncheck`; GitHub runs it on pushes, pull requests, weekly, and on demand with read-only repository permissions. | Automated scanners miss logic flaws and vulnerabilities not yet in their database. Human review remains required for boundary changes. |
...
| T18 | Child receives unsafe AI or broadcast content | The open-ended AI conversation is disabled by default and requires a server-operator child-safety/ZDR gate plus adult-host confirmation. An open request must pass current, read-only OpenAI organization and every-party-project ZDR checks at startup and in the deployment doctor; denial, a non-ZDR override, unknown state, timeout, or malformed evidence fails closed. New projects are checked before their runtime key is saved. The gate is independently enforced in durable settings, web updates, generated routing, FastAGI/key authorization, AudioSocket, and the Realtime bridge. The AI has child-directed instructions and disclosure, exposes no tools, and has bounded calls. Radio choices are fixed. | The gate remains closed until an external child-safety review and OpenAI Zero Data Retention confirmation are complete. Provider status can change after a successful process start, so the operator must re-run the doctor and revoke approval when provider or use conditions change. Live radio is not guaranteed child-appropriate. Open gap. |
...
the external child-safety and real-hardware gates.
