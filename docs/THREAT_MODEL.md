...
| T18 | Child receives unsafe generated or broadcast content | The open-ended conversation line, adult-extension control, media bridge, and route are removed. Remaining provider speech is code-controlled operator/weather text or a fixed join sentence containing the authenticated member label; no caller audio reaches the provider. Radio choices are source-controlled. | OpenAI receives fixed prompt text, weather labels, and joining member labels, and provider retention may apply. Live radio is not guaranteed child-appropriate. Hosts remain responsible for display names and station choice. |
