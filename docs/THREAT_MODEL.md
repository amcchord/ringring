...
| T18 | Child receives unsafe AI or broadcast content | AI is disabled by default, requires adult-host confirmation, has child-directed instructions and disclosure, exposes no tools, and has bounded calls. Radio choices are fixed. | External child-safety review and OpenAI Zero Data Retention confirmation are mandatory before use by callers under 13. Live radio is not guaranteed child-appropriate. Open gap. |
