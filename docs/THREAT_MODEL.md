...
Review baseline: 2026-08-24. Re-review whenever authentication, public ports,
...
6. Bound expensive authentication and provider-backed speech without making ordinary
...
- live family voice media, fixed TTS prompt material, and ephemeral synthesized audio;
...
Phone -- SIP TLS or UDP --> Asterisk -- private AMI/FastAGI --> app
...
- Asterisk is the only public SIP/RTP component. AMI, FastAGI, and
...
| T09 | Invitation or phone setup link/QR is replayed, logged, or grants excess access | Tokens have 32 random bytes, are stored hashed, expire, and are atomically consumed once. Both QR images are rendered in memory without a third-party service. The host invitation reveal is consumed once on a no-store authenticated page; a host-scoped transaction can count and cancel only that party's active unused links without selecting hashes. Only a successful claim response gets a reduced first-call snapshot of dialable display labels/extensions and enabled lines; the invite form does not. Linphone XML returns one device's technical fields only. The vendor-neutral JSON API additionally receives a bounded setup menu made only from active members and routable services in that device's party, excluding itself; server and client validate labels and hidden dial targets. Its canonical and released-iOS compatibility paths share the same token and rate limit. Credential responses refuse `HEAD`, caching, browser CORS, and cross-origin resource use; generic problem responses never classify or echo the token. Token-bearing routes are represented by templates in logs. The public OpenAPI document embeds fictitious data only. Later menu refresh uses the revocable device credential, not the bearer token. | Anyone who obtains an unused bearer link or photograph of its QR before the recipient can consume it. The host must share it privately and cancel active links or rotate a claimed device after suspected disclosure. A successful phone app receives the same party labels/extensions needed to place calls. A malicious native app selected by the recipient can exfiltrate the issued SIP credential; app trust remains part of the private handoff. |
...
| T11 | Radio/weather input becomes SSRF or command/config injection, or one member changes another member's place | Radio stores a catalog identifier and both the web layer and renderer resolve it to a fixed source-controlled URL. Weather uses a bounded host-entered place or exactly five DTMF ZIP digits through an HTTPS API client. Phone setup authenticates the endpoint inside its party, resolves the owning member in storage, rechecks the party enablement, and can fill only that member's unknown location; host writes prove party ownership and member scope in the update. Input never becomes a URL, shell command, or Asterisk application name. | Open-Meteo learns the requested place. OpenAI receives fixed setup text and the resolved label in fixed forecast speech. A party host can see and edit members' weather places. External providers may be unavailable or compromised. Radio content is not moderated by RingRing. |
...
| T16 | Expensive authentication, provider-backed speech, or malformed traffic exhausts the service | Source/category limits, username limits, bounded request bodies, authentication worker slots, provider spend limits, cached fixed prompts, Asterisk limits, and Fail2Ban constrain common paths. | The single host has no upstream DDoS protection and in-memory limits are not multi-node. Availability is best effort. |
...
| T18 | Child receives unsafe generated or broadcast content | The open-ended conversation line, adult-extension control, media bridge, and route are removed. Remaining provider speech is code-controlled operator/time/weather text or a fixed join sentence containing the authenticated member label; no caller audio reaches the provider. Radio choices are source-controlled. | OpenAI receives fixed prompt text, the current local time/date, weather labels, and joining member labels, and provider retention may apply. Live radio is not guaranteed child-appropriate. Hosts remain responsible for display names and station choice. |
...
| T21 | Misdialed digits or an idle handset inject content, cross party boundaries, or create unbounded provider work | Asterisk maps exact operator/help routes and party-local fallback patterns to one of four code constants. FastAGI validates the server-selected party ID, loads only that party's key/settings, never accepts caller text or audio, starts a bundled local greeting immediately, and caches versioned remainder prompts. Renderer and voice tests assert party isolation, fixed prompts, optional-line filtering, pre-roll ordering, and bundled fallback. | Repeated calls can still consume local media and a cache miss can incur one TTS request per party/reason/version. Existing SIP authentication, spend limits, and provider pause controls bound access and cost. |
| T22 | Live-call discovery or joining exposes another party, creates a call log, silently records audio, or lets an announcement inject AMI/dialplan content | Answered calls use validated party/extension-derived ConfBridge names and exact same-party join routes. Dialing the active extension or its hidden `*16…` route enters the same party-scoped join path. Current-channel and conference AMI results are reduced to active PJSIP endpoints mapped through current party membership, four normalized activity states, and bounded elapsed seconds; caller ID, channel/bridge IDs, dialed digits, application data, exact timestamps, and unknown endpoints are dropped. Host ownership is rechecked before every no-store fragment, and the browser timer advances only the reduced duration already in that fragment. ConfBridge recording and CDR are disabled. FastAGI resolves the authenticated endpoint to the joining member and supplies only a fixed name sentence. AMI application, Local channel, conference variable, and playback path are fixed or allowlisted; random-filename name announcements expire after one minute and a startup sweep clears interrupted leftovers. Unit, contract, and a disposable three-phone SIP gate cover cross-party rejection, identifier omission, live rendering, joining, original-owner teardown, metrics, and zero residual channels/CDR. | The authenticated party host and configured same-party phone app can see which party phone is off hook, calling, ringing, or in a call, who is in an active party call, and its live elapsed duration. An ATA exposes no silent pre-dial off-hook state until it sends an INVITE. Anyone with a working phone credential in that party can join a displayed call. OpenAI receives the joining display name, and provider retention may apply. A compromised app container retains narrow AMI call/reporting/originate rights. These are explicit party capabilities; root/Asterisk compromise remains out of scope. |
| T23 | Background-call push leaks family data, lets another party wake a phone, retains a revoked token, or violates CallKit timing | Push registration requires one active device's existing credential over HTTPS; the token is hashed, encrypted with device-bound associated data, and selected only after FastAGI re-authenticates Asterisk's caller endpoint inside the target party. Rotation, revocation, deletion, disconnect, PushKit invalidation, and permanent APNs rejection remove it. The APNs payload contains only an empty `aps` object and opaque UUID. iOS reports that UUID to CallKit immediately, completes the PushKit callback, refreshes SIP, and treats only the later SIP invitation as authoritative. Unit tests cover caller/party scope, token encryption/deletion, minimized provider requests, and invalid registrations; the signed app build is inspected for its production entitlement. | Apple observes push delivery metadata, and a stolen APNs provider key can send opaque wakes to known tokens until it is revoked. The key remains root-owned and is readable only by the unprivileged RingRing app group through a read-only mount. Real background and lock-screen timing depends on iOS, APNs, network conditions, and physical-device acceptance. Explicit force-quit may suppress launch. |
