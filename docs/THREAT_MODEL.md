...
| T04 | Attacker registers as a family phone | Each device has a global random SIP identity and password. Authenticated username precedes IP/anonymous identification. Fail2Ban and separate UDP/TLS limits slow online guessing; rotation and revocation regenerate live configuration. | UDP SIP exposes credentials to an on-path observer; old adapters may have weak TLS support. Prefer TLS. |
...
| T08 | Cross-site request forgery or session theft | All state-changing host routes use POST, authenticate server-side sessions, validate a session-derived CSRF value, and enforce same-origin when Origin is present. Auth forms use a strict same-site double-submit token. Cookies are Secure, HttpOnly, and SameSite; responses containing secrets use `no-store`. OAuth uses random state and PKCE. | Browser or host-device compromise remains out of scope. Native accounts avoid email takeover but depend on saved recovery codes. |
...
| T18 | Child receives unsafe AI or broadcast content | The open-ended AI conversation is disabled by default and requires a server-operator child-safety/ZDR gate plus adult-host confirmation. The closed gate is enforced in durable settings, web updates, generated routing, FastAGI/key authorization, AudioSocket, and the Realtime bridge. The AI has child-directed instructions and disclosure, exposes no tools, and has bounded calls. Radio choices are fixed. | The gate remains closed until an external child-safety review and OpenAI Zero Data Retention confirmation are complete. Approval can become stale and must be revoked if provider or use conditions change. Live radio is not guaranteed child-appropriate. Open gap. |
