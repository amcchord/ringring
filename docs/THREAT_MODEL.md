...
| T22 | Live-call discovery or joining exposes another party, creates a call log, silently records audio, or lets an announcement inject AMI/dialplan content | Answered calls use validated party/extension-derived ConfBridge names and exact same-party join routes. Current-channel and conference AMI results are reduced to active PJSIP endpoints mapped through current party membership, four normalized activity states, and bounded elapsed seconds; caller ID, channel/bridge IDs, dialed digits, application data, exact timestamps, and unknown endpoints are dropped. Host ownership is rechecked before every no-store fragment, and the browser timer advances only the reduced duration already in that fragment. ConfBridge recording and CDR are disabled. FastAGI resolves the authenticated endpoint to the joining member and supplies only a fixed name sentence. AMI application, Local channel, conference variable, and playback path are fixed or allowlisted; random-filename name announcements expire after one minute and a startup sweep clears interrupted leftovers. Unit, contract, and a disposable three-phone SIP gate cover cross-party rejection, identifier omission, live rendering, joining, original-owner teardown, metrics, and zero residual channels/CDR. | The authenticated party host can see which party phone is off hook, calling, ringing, or in a call, who is in an active party call, and its live elapsed duration. An ATA exposes no silent pre-dial off-hook state until it sends an INVITE. Anyone with a working phone credential in that party can join a displayed call. OpenAI receives the joining display name, and provider retention may apply. A compromised app container retains narrow AMI call/reporting/originate rights. These are explicit party capabilities; root/Asterisk compromise remains out of scope. |
