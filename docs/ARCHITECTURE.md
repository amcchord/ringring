...
Every generated party context includes exact `0` and `*0` routes for the RingRing operator. A private FastAGI request carries the server-selected party ID and one fixed reason selected by the dialplan: help, misdial, phone unavailable, or service unavailable. It never carries the dialed number or caller speech. The app validates the party ID, loads that party’s enabled-service snapshot and encrypted runtime key, and asks OpenAI text-to-speech for a code-controlled, clearly disclosed AI-voice explanation. Generated 8 kHz WAV prompts are cached by party, reason, version, and service-update time. When the party key, provider, cache, or private app path is unavailable, the dialplan immediately assembles an appropriate explanation from bundled Asterisk prompts instead.

SIP does not tell RingRing that an analog handset is merely off hook: an ATA sends an INVITE only after it has a destination. A compatible adapter implements the idle-handset experience by using `0` as its delayed off-hook, hotline, or warmline destination. The HT801 V2 setup card uses its documented **Off Hook Auto Dial** setting with an eight-second delay; callers can also dial `0` or `*0` directly from any phone.
