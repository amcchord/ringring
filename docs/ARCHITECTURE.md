...
Every party context ends with numeric and star-code fallbacks that answer and assemble a short explanation from bundled Asterisk prompts. A disabled or unknown destination therefore sounds like a completed private call instead of returning a bare SIP failure tone. Ordinary member routes also inspect `DIALSTATUS`: an offline, busy, or unanswered set of party endpoints gets a spoken retry-later response, while a completed conversation hangs up without the fallback. Private FastAGI failures and a radio player that returns unexpectedly receive the same service-level explanation. Exact member and enabled-service routes remain more specific than these patterns, and every fallback stays inside the authenticated caller's party context.
