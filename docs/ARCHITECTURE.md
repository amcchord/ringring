...
For `*14`, an explicit deployment-wide `AI_CHILD_SAFETY_APPROVED=true` triggers bounded, read-only administrator requests to OpenAI's organization and project data-retention endpoints before RingRing opens a listener. The organization must report `zero_data_retention` or `enhanced_zero_data_retention`; each stored party project must explicitly report a ZDR mode or `organization_default` under that already-verified organization. A missing administrator key, forbidden/not-eligible response, modified-abuse-monitoring or `none` mode, unknown object/value, malformed payload, timeout, or transport failure stops the app. The operator can run the same complete check independently, and `ringringctl doctor` repeats it whenever the configured gate is open. Newly provisioned projects are checked at both levels before their party credential is saved and are archived if either check fails. The administrator key is not used for a model request.

After that precondition, startup closes stale durable settings unless the gate is explicitly true. The reconciler independently filters the route while closed. When approved, FastAGI validates the party setting and issues a short-lived, one-use ticket keyed by a random call UUID. Asterisk then opens an [AudioSocket](https://docs.asterisk.org/Configuration/Channel-Drivers/AudioSocket/) connection containing that UUID. The app claims the ticket, rechecks both the gate and party, decrypts only that party's key, converts Asterisk's 8 kHz signed-linear audio to G.711 μ-law, and bridges it to OpenAI's server-side [Realtime WebSocket](https://developers.openai.com/api/docs/guides/realtime-websocket). The bridge itself checks the gate again before any provider connection. The connection sends a stable privacy-preserving safety identifier but no member name, SIP username, or extension. Neither the organization admin key nor another party's key enters the model call path.
