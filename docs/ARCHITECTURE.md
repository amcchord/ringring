...
A member may have several phones on one extension. The host-only add-device transaction requires the exact party, host, and member together, caps the member at eight saved devices, creates a new random SIP identity and encrypted password, and issues a distinct one-use provisioning token without rotating any existing phone. The renderer expands only that party member's active endpoints into one explicit `Dial(PJSIP/device-a&PJSIP/device-b,30)` destination. Each phone can be tested, rotated, or revoked separately; a disconnected endpoint is omitted on the next reconciliation.
