...
Record the exact vendor, model, firmware/app version, transport, certificate behavior, NAT type, registration duration, incoming and outgoing call result, two-way audio result, and any reboot or renewal behavior. A physical row should cover at least one call to another remote network. Mobile apps additionally need foreground ringing, background/push ringing, and Wi-Fi/cellular transitions before they count toward the family-hardware gate.
