...
The RingRing operator is one-way text-to-speech, not an open-ended conversation. Its help, misdial, phone-unavailable, and service-unavailable scripts are fixed in code, clearly disclose that the voice is AI-generated, and mention optional lines only from the caller’s party-scoped service snapshot. OpenAI receives only that fixed script through the party runtime key—never caller audio, dialed digits, party/member names, extensions, or SIP credentials. Cached audio is derived state, records no caller activity, and is invalidated when relevant service settings change. A missing or paused key, provider failure, or private FastAGI failure falls back to local Asterisk prompts.
