...
RingRing stores a host name and username, member display labels, extensions, device labels, and operational timestamps. An email address is neither requested nor required by native accounts. It does not record calls or send caller audio to its current provider integrations. Synthesized operator and weather files are derived cache data, and short name announcements are deleted after use. Provider-side API retention is controlled separately by the OpenAI organization's data controls. The default frontend includes no advertising, behavioral analytics, or third-party trackers.
...
Active phone and party-call state is likewise visible only on the authenticated host page and is never stored. A no-store fragment polls private AMI, reduces current PJSIP channels to `off hook`, `calling`, `ringing`, or `on a call`, and maps only authenticated endpoints back to active devices in the host's exact party. Validated RingRing conferences add same-party display labels, phone count, a fixed `*16…` join code, and bounded elapsed seconds. The browser advances those duration counters locally between three-second snapshots. RingRing drops caller ID, channel and bridge IDs, addresses, dialed digits, application data, exact start timestamps, unknown endpoints, non-PJSIP announcement channels, foreign rooms, and call history. An ATA that has not sent an INVITE exposes no pre-dial off-hook state. Ordinary answered calls and later join legs explicitly disable CDR, and every ConfBridge profile has recording disabled.
...
Phone setup QRs are rendered inside RingRing and contain one-time HTTPS URLs rather than SIP passwords. Linphone XML returns only the generated SIP identity, password, extension, registrar, and transport needed by that device. The vendor-neutral phone API additionally returns a reduced same-party button snapshot, and its configured-device endpoint can refresh that snapshot and add currently joinable same-party calls. Neither format returns a host identity, device label, email, provider identifier, or OpenAI credential, and no third-party QR/provisioning service is involved.

An iPhone VoIP token is accepted only from an active device authenticated with its existing SIP username and password over HTTPS. The token is hashed for deletion, encrypted with the master key and device-bound associated data for APNs delivery, and removed with the device lifecycle. Incoming-call FastAGI authenticates Asterisk's caller endpoint inside the stated party before selecting only registrations for the same-party target extension. Apple receives an opaque UUID and empty `aps` object—never a party/member name, extension, SIP identity or password, network address, call audio, or transcript. The APNs P-256 provider key is root-owned, mode `0440`, and exposed read-only only to the unprivileged RingRing app group (GID `10001`); it is included only in root-only deployment backups and never enters the checkout or app database.
...
- The downloadable WP826 XML contains one device's SIP password and therefore receives attachment, `no-store`, no-referrer, same-origin-resource, generic-error, masked-log-path, and one-use-token treatment. Its allowlisted alias set configures Account 1, TLS validation, the RingRing asset paths, ringtones, wallpaper, idle keys, and a fixed HTTPS phonebook path authenticated by that same credential; it cannot carry Wi-Fi, network, administrator, party/member label, or unrelated account state. The setup page tells the recipient to upload it directly and delete it afterward.
- The Grandstream phonebook endpoint is no-store, non-CORS, HTTP Basic authenticated over the deployment's existing HTTPS origin, and party-scoped only after resolving the random SIP identity. It returns validated XML labels and dial targets for other active members and enabled `*` services—never the device credential, its own extension, live-call state, foreign-party data, host/email fields, presence, or timestamps. Credential rotation, revocation, and deletion invalidate access without a second persistent token.
- The vendor-neutral phone API returns only one device's validated SIP account plus a bounded snapshot of active, same-party member labels and enabled call services. Its configured-phone state endpoint reuses that device credential and can add reduced live-call buttons but no call history, channel identifier, address, or foreign-party state. It excludes the device's own member and every host, party, device, email, provider, readiness, presence, and timestamp field. The RingRing app stores the snapshot and credential together in the device-only Keychain and renders labels instead of dial targets on its primary call screen. Third-party client requirements are documented in [Phone API](PHONE_API.md).
- Apple VoIP push tokens are device-authenticated, encrypted at rest, content-minimized in transit, and deleted on invalidation, revocation, rotation, or device removal. The push UUID is a wake/correlation hint only; the authenticated SIP invitation remains authoritative and supplies any friendly caller label.
...
- The AMI account is ACL-restricted to the fixed app-container address. Its system/command rights support reload and aggregate status; call/reporting reads list current channels, validated ConfBridge rooms, and PJSIP participants; and its narrow `originate` privilege can reach only the application client's fixed setup-ring or join-announcement shapes. It has neither `all` nor configuration permission, and AMI is never exposed through HTTP.
...
The weather line sends a bounded host-entered place or first-call five-digit U.S. ZIP to Open-Meteo and a short forecast sentence containing the resolved label to the party's OpenAI project for speech generation. It names Open-Meteo as the forecast source. The location belongs to the authenticated endpoint's member, not the party: every phone on one extension shares that member value, while other extensions remain independent. First-call setup accepts only DTMF from Asterisk's authenticated same-party endpoint, can fill only that member's unknown location, rechecks the host's enabled/setup preference in storage, and never replaces a host choice. RingRing does not record or transcribe weather callers and sends no caller audio, member name, extension, or SIP credential to either service. The host can change or clear each member's resolved place from the member gear, or disable the party weather route for everyone.

The RingRing operator is one-way text-to-speech. Its help, misdial, phone-unavailable, and service-unavailable scripts are fixed in code and mention optional lines only from the caller’s party-scoped service snapshot. Asterisk supplies the authenticated endpoint rather than caller ID. OpenAI receives only the selected fixed script through the party runtime key—never caller audio, dialed digits, party/member names, extensions, or SIP credentials. A short bundled greeting begins immediately while the remainder is loaded or generated. Cached audio is derived state, records no caller activity, and is invalidated when relevant service settings change. A missing or paused key, provider failure, or private FastAGI failure falls back to local Asterisk prompts.
...
The former open-ended conversation line has no web control, invitation classification, dialplan route, media listener, or model bridge. Startup clears its legacy enablement field, and the old additive database columns remain only so forward-only SQLite migrations and rollback remain safe.
