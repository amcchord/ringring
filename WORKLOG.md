...
- Creating an isolated production acceptance party exposed a separate provider transition: OpenAI accepted the exact hard monthly limit but reported its enforcement as `inactive` for more than a minute. RingRing correctly refused to issue a runtime key, but did not archive the partially created external project. The exact disposable projects were identified by their test-only name and archived; no confirmed test project remains active.
- The second real SIP client call revealed why both the language-prefixed and root copies still appeared missing: the guarded production checkout uses `umask 077`, Docker preserved that mode on source directories, and the Asterisk sound roots became `0700 root:root`. The unprivileged Asterisk process could stat neither path even though root-run deployment checks could.
...
- Made every packaged sound directory explicitly traversable and every sound file readable, independent of the source checkout's umask. The image retains root ownership for bundled prompts and grants no write access to Asterisk.
...
- Made OpenAI party provisioning transactional across external steps: once a project exists, any spend-limit, service-account, credential-response, cancellation, or timeout failure attempts a bounded archive using a context independent of the canceled request. Archive responses that do not immediately echo status now receive a provider read-back before RingRing decides whether cleanup succeeded.
...
- Focused provider tests prove spend-limit, service-account, and omitted-credential failures archive their partial project, while an ambiguous archive response succeeds only after an authenticated follow-up reports `archived`. Current official OpenAI documentation specifies project creation, hard monthly USD limits in cents with either `inactive` or `enforcing` status, default service-account API-key issuance, and irreversible project archiving through the endpoints RingRing uses.
