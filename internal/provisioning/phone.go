...
func validatePhoneDestination(destination PhoneDestination) error {
	if destination.Kind != "person" && destination.Kind != "service" {
	...
	if !extensionPattern.MatchString(destination.Dial) && !serviceExtensionPattern.MatchString(destination.Dial) {
