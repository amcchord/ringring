...
`make sip-smoke` builds RingRing plus the pinned Asterisk and SIPp images, seeds a disposable two-phone database, and places the complete app, PBX, and clients on an internal Docker network. It verifies registration, extension calling, `*10` audio, and an authenticated `*15` DTMF interaction that changes `101` to `103`, preserves the SIP credential, regenerates configuration, and reloads Asterisk. It reads no application environment or production database, publishes no host ports, and removes its containers, network, database, and generated state when finished.
