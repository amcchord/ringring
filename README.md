...
Every party has a private RingRing operator on `0` and `*0`, plus a `*10` echo test that lets one phone prove its microphone, speaker, and two-way media path. The energetic operator is generated with the party’s OpenAI text-to-speech key, clearly identifies itself as an AI voice, and gives a short tour using only services enabled for that party. The same fixed, code-controlled voice explains misdialed numbers and unavailable phones or services. RingRing sends no caller audio, dialed digits, names, or SIP credentials for these prompts, caches the generated WAV files, and falls back to bundled Asterisk audio whenever OpenAI is unavailable. A compatible ATA can auto-dial `0` after an 8–10 second off-hook delay.
